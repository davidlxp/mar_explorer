...
- POTENTIAL SPEED IMPROVMENT: Consider architecting a system that generates multiple tasks at the same time which can be done in-parallel (No dependency). So, we can run it in parallel. If the 1st batch tasks work, we can hit the aggregator right away. If it doesn't work, we can carry the results to ask planner to plan for the next set of tasks.
